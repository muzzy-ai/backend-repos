...
const listOrders = async (req, res) => {
  try {
    const orders = await orderModel.find({}).sort({ date: -1 }); // Urutkan descending (terbaru di atas)
    res.json({ success: true, data: orders });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: "Error" });
...
const updateStatus = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Status berhasil diperbarui", data: updatedOrder });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: "Terjadi kesalahan dalam memperbarui status" });
  }
};
